...
class OLVQ1:
    ...
    def fit(self, X, y):
        ...
        self.prototypes_lr = np.ones(len(self.prototypes)) * self.learning_rate

        for epoch in range(self.n_epochs):
            for sample, label in zip(X, y):
                dist = np.linalg.norm(self.prototypes - sample[None], axis=-1)
                closest_idx = np.argmin(dist)
                eta = self.prototypes_lr[closest_idx]
                if self.prototype_labels[closest_idx] == label:
                    self.prototypes[closest_idx] += eta * (sample - self.prototypes[closest_idx])
                    self.prototypes_lr[closest_idx] = eta / (1 + eta) # decrese learning rate
                else:
                    self.prototypes[closest_idx] -= eta * (sample - self.prototypes[closest_idx])
                    self.prototypes_lr[closest_idx] = np.clip(eta / (1 - eta + 1e-6), 1e-4, 1.0) # increase learning rate
...
if __name__ == "__main__":
    ...
    model = OLVQ1(n_prototypes_per_class=3, learning_rate=0.01, n_epochs=20)
    model.fit(X, y_true)
    ...
    plt.scatter(model.prototypes[:, 0], model.prototypes[:, 1], c=model.prototype_labels, s=50, marker='o', edgecolor='k', cmap='jet')
    plt.title("OLVQ Clustering")
    plt.show()