...
class MultiVarDecisionTree:
    ...
    def draw_classifier(self, X, y, classifier, pclassifier, node_id=0, pnode=None):
        ...
        w = classifier.W[:-1].squeeze()
        b = classifier.W[-1]
        y_points = -(w[0] * x_points + b) / w[1]
        
        plt.plot(x_points, y_points, "b-", label=f"node {node_id}")
        if pclassifier is not None:
            pw = pclassifier.W[:-1].squeeze()
            pb = pclassifier.W[-1]
            parent_y_points = -(pw[0] * x_points + pb) / pw[1]
            plt.plot(x_points, parent_y_points, "r-", label=f"pnode {pnode}")
    ...
    def _build_tree(self, X, y, pclassifier=None, pnode=None, draw=False):
        ...
        classifier = LogisticRegression(lr=0.1, num_iters=100000)
        classifier.fit(X, y, draw=False)
        if draw:
            self.draw_classifier(X, y, classifier, pclassifier, node_id=node_id, pnode=pnode) 

        proba = classifier.predict(X).squeeze()
    ...
    def predict_sample(self, sample, node=None):
        ...
        if node.is_leaf:
            ...
            proba = node.classifier.predict(sample).squeeze(1) # single sample
